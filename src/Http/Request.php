...
use GeekBrains\LevelTwo\Users\Exceptions\JsonException;

class Request 
{
    public function __construct(
            // создаем явно для отладки
        private array $get, // соответствует $_GET
        private array $server,  // тут скпер глобальная $_SERVER
...
// Метод для получения отдельного поля из json-форматированного тела запроса
    ...
    public function jsonBodyField(string $field): mixed
    {
        ...
        // Метод для получения пути запроса. Напрмер, для http://example.com/some/page?x=1&y=acb путём будет строка '/some/page'
    public function path(): string
    {
        ...
        // Используем встроенную в PHP функцию parse_url парсим
